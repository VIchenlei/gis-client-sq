let reptQuery = {
  ...
  person_well_overtime: {
    ...
    fields: {
      names: ['s.card_id', 'ds.name', 'ifnull(d.name, " ")', 's.occupation_id', 'hed.map_id', 'ifnull(date_format(ras.start_time, "%Y-%m-%d %H:%i:%s"), " ") as start_time', 'hed.cur_time', 'ifnull(date_format(hed1.cur_time, "%Y-%m-%d %H:%i:%s"), " ") as end_time', 'hed.limit_value', 'round(hed.cur_value,2)', 'case when aa.card_id is null then "否" else "是" end as isbat'],
      types: ['NUMBER', 'STRING', 'STRING', 'SELECT', 'SELECT', 'DATETIME', 'DATETIME', 'DATETIME', 'STRING', 'STRING', 'STRING'],
      labels: ['卡号', '姓名', '所属部门', '职务', '所属地图', '入井时间', '开始告警时间', '结束告警时间', '规定时长(分)', '实际时长(分)', '充放电']
    },
    exprFields: [{
      name: 'ds.staff_id',
      label: '员工编号',
